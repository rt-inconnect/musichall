...
module.exports.sockets = {
  ...
  onConnect: function(session, socket) {
    if(session.passport && session.passport.user) {
      sails.sockets.join(sails.sockets.id(socket), 'user#' + session.passport.user);
      sails.sockets.join(sails.sockets.id(socket), 'onliners');

      User.update({id: session.passport.user}, {isOnline: 1}).exec(function (err, user) {
        if (err) sails.log.error('sockets.js:onConnect', 'User.update', session.passport.user, err);
        sails.sockets.broadcast("onliners", 'online', user);
      });
  ...
  onDisconnect: function(session, socket) {
    if(session.passport && session.passport.user) {
      sails.sockets.leave(sails.sockets.id(socket), 'user#' + session.passport.user);
      sails.sockets.leave(sails.sockets.id(socket), 'onliners');

      User.update({id: session.passport.user}, {isOnline: 0}).exec(function (err, user) {
        if (err) sails.log.error('sockets.js:onDisconnect', 'User.update', session.passport.user, err);
        sails.sockets.broadcast("onliners", 'offline', user);
      });
    }
